...
export function usePurchaseTickets() {
  ...
  // For approval
  const { writeContract: approveWrite, data: approveTx, isPending: isApproving, error: approveError } = useWriteContract();
  const { isSuccess: isApproved } = useWaitForTransactionReceipt({ hash: approveTx });
