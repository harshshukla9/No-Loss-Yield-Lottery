...
const PurchaseTickets = () => {
  const { ticketPurchaseCost, userAddress } = useLottery();
  ...
  const {
    ...
    data: approveTx,
    reset: resetApprove,
    isPending: approvingSubmission,
  } = useWriteContract();
  const { isSuccess: approved, isLoading: isApprovingTxPending } =
  ...
  const {
    ...
    data: stakeTx,
    reset: resetStake,
    isPending: stakingSubmission,
  } = useWriteContract();
  const { isSuccess: staked, isLoading: isStakingTxPending } =
  ...
  const isApproving = approvingSubmission || isApprovingTxPending;
  const isStaking = stakingSubmission || isStakingTxPending;

  const isInputDisabled = isApproving || isStaking;
  let isApproveDisabled =
    !ticketAmount || !!error || isApproving || !ticketPurchaseCost.data;
  let isStakeDisabled =
    !ticketAmount ||
    !!error ||
    isStaking ||
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-3">
            ...
            >
              {isStaking ? "Staking..." : staked ? "✓ Staked" : "2. Stake LINK"}
            </button>
          </div>
        </div>
      </div>
